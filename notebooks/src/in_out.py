...
def files_in_dirs(top_dir):
    print(top_dir)
    full_name = glob.glob(top_dir + "/*.txt")
...
def load_txt(file_name):
    src = np.loadtxt(file_name)
    print(src.shape)
    normlized_xyz = np.zeros((8192, 3))
    coord_min , coord_max = np.amin(src, axis=0)[:3], np.amax(src, axis=0)[:3]
    if(coord_max[0]==0):
        src = np.loadtxt("/home/7/19D50097/shino/3dgan/data/point/UH_NAD83_273248_3289689.93.txt")
        coord_min , coord_max = np.amin(src, axis=0)[:3], np.amax(src, axis=0)[:3]
    if(coord_max[1]==0):
        src = np.loadtxt("/home/7/19D50097/shino/3dgan/data/point/UH_NAD83_273248_3289689.93.txt")
        coord_min , coord_max = np.amin(src, axis=0)[:3], np.amax(src, axis=0)[:3]
    if(coord_max[2]==0):
        src = np.loadtxt("/home/7/19D50097/shino/3dgan/data/point/UH_NAD83_273248_3289689.93.txt")
        coord_min , coord_max = np.amin(src, axis=0)[:3], np.amax(src, axis=0)[:3]
    src[:, 0] = src[:, 0] / coord_max[0]
    src[:, 1] = src[:, 1] / coord_max[1]
    src[:, 2] = src[:, 2] / coord_max[2]
    if(len(src) >=8192):
        np.random.shuffle(src)
        normlized_xyz[:,:]=src[:8192,:]
    else:
        normlized_xyz[:len(src),:]=src[:,:]
        
    return normlized_xyz
...
def load_all_point_clouds_under_folder_txt(top_dir, n_threads=20, file_ending='.txt', verbose=False):
    top_dir = "/home/7/19D50097/shino/3dgan/data/point/"
    file_names = [f for f in files_in_dirs(top_dir)]
    pclouds, model_ids, syn_ids = load_point_clouds_from_filenames_txt(file_names, n_threads, loader=txt_loader, verbose=verbose)
    return PointCloudDataSet(pclouds, labels=None, init_shuffle=False)
...
def load_point_clouds_from_filenames_txt(file_names, n_threads, loader, verbose=False):
    print(file_names)
    pc = loader(file_names[0])[0]
    pclouds = np.empty([len(file_names), pc.shape[0], pc.shape[1]], dtype=np.float32)
    model_names = np.empty([len(file_names)], dtype=object)
